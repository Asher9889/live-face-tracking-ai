...
class FaceLandmarkerEngine:
    ...
    def score_face(self, a: Dict, debug=False) -> float:
        ...
        # If pose missing → degrade, don't reject
        if yaw is None or pitch is None:
            return 0.3
        ...
        # ---------------- PARTIAL FACE ---------------- #
        eye_ratio = a.get("eye_dist_ratio")
        ...
        # ---------------- MILD BLUR PENALTY ---------------- #
        blur = a.get("blur")
        if blur is not None and blur < (self.blur_threshold * 1.5):
            score *= 0.8

        if debug:
            print(f"[MediaPipe] score={score:.2f}")
